...
class Task():
    def __init__(self, groupClass, title, dueDate, progress = 0, completed = 0, id = 0):
        ...
        self.expectedProgress = taskController.expected_progress(daysLeft)
        color = fg('deep_pink_1b')
        bold = attr('bold')
        reset = attr('reset')

        if self.expectedProgress > 100 or self.expectedProgress <= 0: # if overdue or still have some time before needing to do it 
            self.expectedProgressTxt = ""
            self.expectedProgressTxtNormal = ""
        else:
            self.expectedProgressTxt = color + str(self.expectedProgress) + bold + reset
            self.expectedProgressTxtNormal = str(self.expectedProgress)
